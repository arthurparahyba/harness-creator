...
| 2.6 | Escolher a variante de `<como-propor-mudanca-de-plano>`: as duas fontes (com `openspec/`) ou só TASKS.md | `02-preenchimento-templates.md` | Mandar usar o fluxo do OpenSpec num repo que não o tem é instruir o agente a chamar o que não existe. E onde as duas existem, variante sem critério faz a escolha cair na ordem dos arquivos. As skills (`openspec-propose`, `openspec-apply-change`) têm o mesmo nome nos três agentes-alvo; o comando, não |
