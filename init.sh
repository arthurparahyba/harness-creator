...
# go mod download             # Go
# cargo fetch                 # Rust
# `python3 -m pip` e não `pip`: em muitas instalações o executável `pip` não
# existe no PATH, e com `set -e` isso derruba a sessão inteira no passo 1.
PY="${PYTHON:-python3}"
"$PY" -m pip install -q pytest ruff mypy pyyaml types-PyYAML
...
# go test ./...               # Go
# cargo test                  # Rust
"$PY" -m pytest -q || echo "AVISO: falhas pré-existentes acima — registrar, não consertar"
